...
class Vacancy:
    ...
    def __repr__(self):
        return f'{self.__class__.__name__}({self.name}, {self.company_name}, ' \
               f'{self.url}, {self.description}, {self.salary}, {self.experience})'
...
class HHVacancy(Vacancy):  # add counter mixin
    ...
    def __str__(self):
        return f'HH: {self.company_name}, зарплата: {self.salary} руб/мес, ' \
               f'ссылка: {self.url}, требуемый опыт: {self.experience}, описание: {self.description[:50]}'
...
def sorting(vacancies: list):
    ...
    for i in range(len(vacancies)):
        ...
        for j in range(i + 1, len(vacancies)):
            if vacancies[j] < vacancies[minimum]:
                minimum = j
        vacancies[minimum], vacancies[i] = vacancies[i], vacancies[minimum]
    return vacancies
