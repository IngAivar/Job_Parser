...
class Vacancy:
    __slots__ = ('title', 'salary_min', 'salary_max', 'currency',
    ...
    def select(self):
        with open(self.__filename, 'r', encoding='utf-8') as file:
            ...
            for row in data:
                ...
                if row['salary']:
                    salary_min, salary_max, currency = row['salary']['from'], row['salary']['to'],\
                        row['salary']['currency']
                vacancies.append(Vacancy(row['name'], salary_min, salary_max, currency,
                                         row['employer']['name'], row['alternate_url']))
            return vacancies
